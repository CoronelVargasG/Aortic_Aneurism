...
# Identify the most important proteins for each condition (based on absolute value of loadings)
...
top_proteins <- names(sort(importance, decreasing = TRUE))[1:200]
...
# 4) Second Biplot Showing the Top 200 Most Important Proteins Based on the Elbow Plot's K Value
...
top_proteins_K <- names(sort(importance_K, decreasing = TRUE))[1:200]
...
# Create biplot showing the top 200 most important proteins based on K components
